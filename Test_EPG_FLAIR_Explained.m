...
fig = figure('Name','3D FLAIR EPI test','Color','w','Units','normalized','Position',[0.08 0.08 0.82 0.82]);
tabs = uitabgroup(fig);

% 1. FLAIR inversion recovery: shows longitudinal magnetization Mz after
% inversion, FLAIR only works when CSF is near 0 at chosen TI.
...
% 2. EPI echo train: shows signal during EPI readout. In EPI each echo
...
hold on
for ii = 1:numel(tissue)
plot(echoNumber,real(signals(ii).F(3,:)),'LineWidth',2,'Color',tissue(ii).color)
end
xlabel('EPI echo number')
ylabel('Z_0 during readout')
title('Dynamic EPG state evolution after FLAIR preparation')
...
% 7. TI optimisation: sweeps TI values and checls CSF signal,FLAIR depends criticlly on choosing the right TI 
tab = uitab(tabs,'Title','7 TI optimisation');
axes('Parent',tab)
...
TItest = TI_values(ii);
[F0,~,~,~] = EPG_FLAIR(flipAngle,ESP,tissue(1).T1,tissue(1).T2,TItest,'sequence','epi3d','T2star',tissue(1).T2star,'nPE',nPE,'nPartitions',nPartitions,'effectiveEcho',effectiveEcho);
WM(ii) = abs(F0(effectiveEcho));

[F0,~,~,~] = EPG_FLAIR(flipAngle,ESP,tissue(2).T1,tissue(2).T2,TItest,'sequence','epi3d','T2star',tissue(2).T2star,'nPE',nPE,'nPartitions',nPartitions,'effectiveEcho',effectiveEcho);
GM(ii) = abs(F0(effectiveEcho));

[F0,~,~,~] = EPG_FLAIR(flipAngle,ESP,tissue(3).T1,tissue(3).T2,TItest,'sequence','epi3d','T2star',tissue(3).T2star,'nPE',nPE,'nPartitions',nPartitions,'effectiveEcho',effectiveEcho);
...
xlabel('TI (ms)')
title(sprintf('7. TI optimisation, best TI = %.0f ms',bestNullTI))
