...
%EPG_FLAIR Simulate FLAIR-prepared TSE or 3D EPI signal evolution.
%
...
% FLAIR preparation: 180 degree inversion followed by T1 recovery for TI.
Mz_before_excitation = 1 - (1 + opts.zinit)*exp(-TI/T1);
Mxy0 = sin(alpha)*Mz_before_excitation;
Mz_after_excitation = cos(alpha)*Mz_before_excitation;

echoNumber = 1:nPE;
echoTimes = echoNumber*ESP;
readoutDecay = exp(-echoTimes/T2star);
F0 = Mxy0*readoutDecay;

% Compact EPG state for a gradient-echo readout. EPI gradients dephase and
% rephase transverse coherence for each echo; here the observable F0 signal
% is weighted by T2* during the readout instead of by TSE refocusing pulses.
F = zeros(3,nPE);
F(1,:) = F0;
F(2,:) = conj(F0);
F(3,:) = Mz_after_excitation*ones(1,nPE);
Zn = F(3,:);
...
order = [order candidates]; %#ok<AGROW>
end
otherwise
